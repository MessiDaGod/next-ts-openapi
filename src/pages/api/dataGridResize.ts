...
function setListeners(div: HTMLDivElement): void {
  ...
  // let MaxWidths: Array<number> = []; // create an object to store the max cell width for each column

  // rows.forEach((row) => {
  //   const cells = row.querySelectorAll('div[class*="' + 'td' + '"]'); // get all cells in current row

  //   cells.forEach((cell) => {
  //     if (cell.getAttribute('hidden') !== null) return;
  //     const columnId =
  //       (cell as HTMLDivElement).getAttribute('data-column-id') ?? '';
  //     let cellWidth = (cell as HTMLDivElement);
  //     cellWidth.setAttribute("style", "width: auto;")
  //     MaxWidths.push(paddingDiff(cellWidth));
  //   });
  // });

  // console.log(Math.max(...MaxWidths));
